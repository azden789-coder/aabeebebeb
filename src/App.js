...
export default function App() {
  ...
  // Sprawdź premium po powrocie ze Stripe
  useEffect(()=>{
    const params = new URLSearchParams(window.location.search);
    const emailParam = params.get("email");
    const premiumParam = params.get("premium");
    // Wyczyść URL
    if(premiumParam || emailParam) window.history.replaceState({}, "", window.location.pathname);
    if(emailParam) checkPremium(emailParam);
    // Wrócił ze Stripe – pokaż ekran premium z komunikatem
