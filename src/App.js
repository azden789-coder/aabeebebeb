  ...
  return (
    <div style={phoneStyle}>
      ...
      <div style={scrollStyle}>
        ...
        {isPremium
          ? <div style={{ margin:"8px 12px", background:"rgba(0,230,118,.1)", border:`1.5px solid ${C.green}`, borderRadius:14, padding:16, textAlign:"center" }}>
              <div style={{ fontSize:36, marginBottom:6 }}>🎉</div>
              <div style={{ fontSize:17, fontWeight:900, color:C.green }}>Premium aktywowane!</div>
              <div style={{ fontSize:12, color:C.muted, marginTop:6 }}>📧 {premiumEmail}</div>
              <div style={{ fontSize:12, color:C.muted, marginTop:2 }}>⏳ Ważne do: <strong style={{ color:C.text }}>{expDate}</strong></div>
              <Btn onClick={()=>setScreen("home")} style={{ width:"100%", background:`linear-gradient(90deg,${C.green},${C.greenDark})`, color:"#000", fontSize:15, marginTop:12 }}>
                ✅ Przejdź do typów →
              </Btn>
            </div>
...
const BACKEND_URL = "https://hitkupon.vercel.app";
...
export default function App() {
  ...
  // Sprawdź premium po powrocie ze Stripe (URL ?email=xxx&premium=true)
  useEffect(()=>{
    const params = new URLSearchParams(window.location.search);
    const emailParam = params.get("email");
    const premiumParam = params.get("premium");
    if(emailParam){
      checkPremium(emailParam);
      // Jeśli wrócił ze Stripe, pokaż ekran premium z sukcesem
      if(premiumParam === "true") setScreen("premium");
      // Wyczyść URL żeby nie było brzydkie
